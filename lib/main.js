...
function Slice() {
	...
	if ( !isValid( start ) ) {
		throw new TypeError( format( '1izEf', start ) );
	}
	if ( !isValid( stop ) ) {
		throw new TypeError( format( '1izEg', stop ) );
	}
	if ( !isValid( step ) ) {
		throw new TypeError( format( '1izEh', step ) );
	} else if ( step === 0 ) {
		throw new RangeError( format( '1izEi', step ) );
	}
